...
export default function StudentAnalysis() {
  ...
  const handleViewDetails = async (studentId: string, studentName: string) => {
    ...
    try {
      ...
      setSelectedStudentDetails(details as any);
    } catch (error: any) {
      toast({
        title: 'Error',
      ...
      <Dialog open={detailDialogOpen} onOpenChange={setDetailDialogOpen}>
        <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
          ...
          {loadingDetails ? (
            ...
            <div className="space-y-4">
              {selectedStudentDetails.map((detail) => (
                <Card key={detail.id}>
                  <CardContent className="pt-6">
                    <div className="space-y-4">
                      <div className="flex items-start justify-between">
                        <div>
                          <h3 className="font-semibold text-lg">{detail.exam.title}</h3>
                          <p className="text-sm text-muted-foreground">
                            {detail.exam.class.class_name} • {detail.exam.subject.subject_name}
                          </p>
                        </div>
                      ...
                      <div className="grid gap-4 md:grid-cols-3">
                        <div>
                          ...
                          <p className="font-semibold">
                            {detail.total_marks_obtained !== null ? detail.total_marks_obtained : 'N/A'} / {detail.exam.total_marks}
                          </p>
                        </div>
                        <div>
                          <p className="text-sm text-muted-foreground">Percentage</p>
                          <p className="font-semibold">
                            {detail.percentage !== null ? `${detail.percentage.toFixed(2)}%` : 'N/A'}
                          </p>
                        </div>
                        <div>
                          <p className="text-sm text-muted-foreground">Submitted At</p>
                          <p className="font-semibold text-sm">{formatDateTime(detail.submitted_at)}</p>
                        </div>
                      </div>
                    </div>
                  </CardContent>
